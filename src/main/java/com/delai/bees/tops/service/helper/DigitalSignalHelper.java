 ...
 */
public class DigitalSignalHelper {
    /**
     * 结合
     * @param dss
     * @param scs
     ...
     * @return
     */
    public static List<DigitalSignal> combine(List<DigitalSignal> dss , List<SignalConfig> scs, List<Machine> machines) {
        for (SignalConfig sc : scs ) {
            ...
            // 拼接信号
            for (DigitalSignal ds : dss) {
                if (sc.getId().equals(ds.getSignalId())) {
                    ds.setSignal(sc);
                }
            }
        }
